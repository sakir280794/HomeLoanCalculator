...
export default function HomeLoanPage() {
  ...
  useEffect(() => {
    if (!computedInputs || !result) { setBoostSummary(null); setCombinedSummary(null); return; }
    const roundedBase = Math.ceil(result.base.emi / 100) * 100;
    if (boostState.emi <= roundedBase) { setBoostSummary(null); setCombinedSummary(null); return; }
    const booster: BoosterInputs = { boostedEMI: boostState.emi, startMonth: boostState.month, startYear: boostState.year };
  ...
  const handleLoanField = useCallback(
    (field: keyof LoanFormValues, value: string | number | boolean) => {
      setLoanForm(prev => {
        ...
        if ((field === 'propertyValue' || field === 'downPayment' || field === 'downPaymentType') && !prev.loanAmountManual) {
  ...
  const validate = useCallback((): boolean => {
    ...
    if (prepForm.enabled) {
      const total = (parseFloat(prepForm.monthlyExtra) || 0) + (parseFloat(prepForm.oneTime) || 0) + (parseFloat(prepForm.yearly) || 0) + prepForm.lumpSums.reduce((s, ls) => s + (parseFloat(ls.amount) || 0), 0);
  ...
  const handleCalculate = useCallback(() => {
    ...
    const prepInputs: PrepaymentInputs | undefined = prepForm.enabled ? {
      ...
      startMonth: prepForm.startMonth, startYear: prepForm.startYear, mode: prepForm.mode,
      lumpSums: prepForm.lumpSums.filter(ls => (parseFloat(ls.amount) || 0) > 0).map(ls => ({ amount: parseFloat(ls.amount), month: ls.month, year: ls.year })),
    } : undefined;
    ...
    if (topUpForm.enabled) {
      ...
      setTopUpResult({ amount: ta, emi: tEMI, totalInterest: tInterest, totalPayment: ta + tInterest, tenureMonths: tm, payoffMonth: (absStart % 12) + 1, payoffYear: Math.floor(absStart / 12), interestRate: tr });
      const topUpLoan = calculateLoan({ propertyValue: 0, downPayment: 0, downPaymentType: 'amount', loanAmount: ta, interestRate: tr, tenureYears: parseInt(topUpForm.tenureYears) || 0, tenureMonths: parseInt(topUpForm.tenureMonths) || 0, loanStartMonth: topUpForm.startMonth, loanStartYear: topUpForm.startYear });
      setTopUpScheduleData(topUpLoan.base.schedule);
    } else { setTopUpResult(null); setTopUpScheduleData(null); }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 items-start">
          <div className="space-y-4 lg:sticky lg:top-24">
            <LoanDetailsForm value={loanForm} onFieldChange={handleLoanField} onLoanAmountChange={handleLoanAmountChange} onResetLoanAmount={resetLoanAmount} errors={errors} />
            <PrepaymentSection value={prepForm} onChange={setPrepForm} errors={errors} />
            <TopUpSection value={topUpForm} onChange={setTopUpForm} errors={errors} baseLoanAmount={loanAmount} />
            <button onClick={handleCalculate} className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-bold py-4 px-6 rounded-xl transition-all duration-200 flex items-center justify-center gap-3 shadow-lg text-base active:scale-95">
          ...
          <div ref={resultsRef} className="min-w-0">
            {result ? (
              <div className="space-y-5">
                ...
                <EMIBooster loanAmount={loanAmount} interestRate={parseFloat(loanForm.interestRate) || 0} baseEMI={result.base.emi} loanStartMonth={loanForm.loanStartMonth} loanStartYear={loanForm.loanStartYear} onBoostChange={handleBoostChange} />
                {topUpResult && <TopUpResults result={topUpResult} baseEMI={result.base.emi} />}
                <LoanCharts result={result} />
                <AmortizationTable result={result} loanAmount={loanAmount} emi={result.base.emi} totalInterest={result.base.totalInterest} totalPayment={result.base.totalPayment} topUpSchedule={topUpScheduleData ?? undefined} topUpSummary={topUpResult ? { loanAmount: topUpResult.amount, emi: topUpResult.emi, totalInterest: topUpResult.totalInterest, totalPayment: topUpResult.totalPayment } : undefined} boostSummary={boostSummary ?? undefined} combinedSummary={combinedSummary ?? undefined} />
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center min-h-96 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-gray-200 dark:border-slate-600 p-12">
                ...
                <h3 className="text-lg font-semibold text-gray-400 dark:text-slate-500 text-center">Your results will appear here</h3>
                <p className="text-sm text-gray-300 dark:text-slate-600 text-center mt-2 max-w-xs">Fill in the loan details and click &quot;Calculate EMI&quot; to see the breakdown</p>
                <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3 w-full max-w-md">
                  {[{ label: 'EMI Breakdown', desc: 'Principal + Interest' }, { label: 'EMI Booster', desc: 'Reduce tenure fast' }, { label: 'Top-up Loan', desc: 'Combined EMI view' }, { label: 'Amortization', desc: 'Month-by-month table' }].map(h => (
      ...
      <footer className="mt-12 py-8 border-t border-gray-200 dark:border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500">Results are indicative. Consult your lender for exact figures.</p>
        </div>
      </footer>
    </div>
  );
